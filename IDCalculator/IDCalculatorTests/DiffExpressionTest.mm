...
-(void) testNegativePower {
    ...
    Expression* diff = [[func expression] evaluate];
    XCTAssertEqualObjects([diff description], @"-1/x²", @"");
...
-(void)testComplexPolynomial {
    ...
    Expression* diff = [[func expression]evaluate];
    XCTAssertEqualObjects([diff description], @"(x²+2x-1)/(x²+2x+1)", @"");
}
